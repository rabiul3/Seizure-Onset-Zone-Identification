...
data_FCD2_n,time=data_FCD2[range(0,45)]



chan0_36=data_FCD2_n[0:37,:]
chan39_45=data_FCD2_n[39:45,:]

data_FCD2_n=np.vstack((chan0_36, chan39_45))
...
segmented_frame=np.empty(shape=[43,40000])
stored_filtered_data=np.empty(shape=[43,10,40000])
seg_filtered_data=np.empty(shape=[180,43,10,40000])
...
Temp2=np.empty(shape=[43,10,9])     #channel X subbands X new_features
Temp3=np.empty(shape=[180,43,10,9])  #segments X cnannel X subbands X new_features
...
scipy.io.savemat('Pt3_sheuli_new_fea_0_180epoc_Norm.mat', mdict={'pt3_0_180epoc_sheuliNew_fea': Temp3})        
