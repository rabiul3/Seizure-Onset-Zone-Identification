...
EDF_PATH = '/Volumes/ExternalHDD/intracranial/FCD/00_out_201811_EEG_11698816/'   # This is the path of data of data in Juntendo server
EDF_NAME = EDF_PATH +'DJ9990B9.edf'
...
#number of channel 67 and  start time from 
#1 hour 1s to end 
...
segmented_frame=np.empty(shape=[67,40000])
stored_filtered_data=np.empty(shape=[67,10,40000])
seg_filtered_data=np.empty(shape=[180,67,10,40000])
...
Temp2=np.empty(shape=[67,10,9])     #channel X subbands X new_features
Temp3=np.empty(shape=[180,67,10,9])  #segments X cnannel X subbands X new_features
...
scipy.io.savemat('P5_sheuli_new_fea_0_180epoc_Norm.mat', mdict={'p5_0_180epoc_sheuliNew_fea': Temp3})        
