...
EDF_PATH = '/Volumes/ExternalHDD/intracranial/FCD/00_out_201811_EEG_11295074/'   # This is the path of data of data in Juntendo server
EDF_NAME = EDF_PATH +'FJ3740FG.edf'
...
data_FCD2_n,time=data_FCD2[range(0,83)]
...
segmented_frame=np.empty(shape=[83,40000])
stored_filtered_data=np.empty(shape=[83,10,40000])
seg_filtered_data=np.empty(shape=[180,83,10,40000])
...
Temp2=np.empty(shape=[83,10,9])     #channel X subbands X new_features
Temp3=np.empty(shape=[180,83,10,9])  #segments X cnannel X subbands X new_features
...
scipy.io.savemat('P8_sheuli_new_fea_0_180epoc_norm.mat', mdict={'p8_0_180epoc_sheuliNew_fea': Temp3})        
