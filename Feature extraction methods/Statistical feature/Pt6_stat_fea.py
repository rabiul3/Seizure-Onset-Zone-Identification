...
EDF_NAME = EDF_PATH +'DJ0010MV.edf'
...
data_FCD2_n,time=data_FCD2[range(0,77)]
...
segmented_frame=np.empty(shape=[77,40000])
stored_filtered_data=np.empty(shape=[77,10,40000])
seg_filtered_data=np.empty(shape=[180,77,10,40000])
...
Temp2=np.empty(shape=[77,10,9])     #channel X subbands X new_features
Temp3=np.empty(shape=[180,77,10,9])  #segments X cnannel X subbands X new_features
...
scipy.io.savemat('P6_sheuli_new_fea_0_180epoc_norm.mat', mdict={'p6_0_180epoc_sheuliNew_fea': Temp3})        
